...
4. 재부팅한다.
5. LostArkPad.exe로 프로그램 실행 후 사용한다.
   - 처음 실행시 XBox 360 가상 드라이버를 설치하는데, 패드 모드에서 WASD키로 캐릭터가 움직이지 않을 경우, 한번 더 재부팅을 한다.
6. 프로그램 사용중 키보드 또는 마우스 장치의 변경 및 추가로 인해 인식이 안될 경우에는 Alt + S 를 눌러 디바이스를 다시 인식시켜야 한다.
   - 위의 상황일 경우의 증상은 단축키를 눌러도 아무 반응이 없는 것이다. 
   - 간혹 재부팅 후 디바이스 ID가 변경되는 경우도 있으므로 단축키 반응이 없을 경우 Alt + S를 눌러 디바이스를 인식시켜야 한다.
...
|디바이스 선택  |Alt + S       |X       |             |키보드 마우스 장치 재인식.               |

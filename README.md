...
6. 프로그램 사용중 키보드, 마우스 장치의 변경 및 추가로 인해 인식이 안될 경우에는 Alt + S 를 눌러 디바이스를 다시 인식시켜야 한다.
   - 단축키를 눌러도 아무 반응이 없을 경우, 위의 상황일 확률이 높다. (실제 장치를 변경하지 않더라도 간혹 디바이스 ID가 변경되는 경우가 있다.)

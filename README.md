...
## ◈ 사용 방법
    ...
    → 이동 : 화살표키
      (*관리자 권한으로 실행시 화살표키 방향으로 마우스 커서 이동(위치 지정 스킬 사용시 필요).)
    → 상호 작용 : Wheel Down (인게임 G키.)
    → 달리기 : 더블 클릭 (마우스 방향으로 자동 달리기.)
    → 회피 : 이동 중에 Wheel Up (인게임 Space키.)
    → 말타기 : 이동 중에 MButton (인게임 F3키.)
    → 회복약 : 이동 중이 아닐 때 MButton (인게임 F1키.)
    → 제스처 스킬 (타입 1): MButton(가운데 버튼) 클릭 드래그
        - 스킬 1(Q) : 왼쪽 드래그
        - 스킬 2(W) : 위로 드래그
        - 스킬 3(E) : 아래로 드래그
        - 스킬 4(R) : 오른쪽 드래그
    → 제스처 스킬 (타입 2): XButton1(뒤로 가기 버튼) 클릭 드래그
        - 스킬 5(A) : 왼쪽 드래그
        - 스킬 6(S) : 위로 드래그
        - 스킬 7(D) : 아래로 드래그
        - 스킬 8(F) : 오른쪽 드래그
